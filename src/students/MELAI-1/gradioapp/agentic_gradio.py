...
def gradio_generate_report(
    topic: str, temperature: float, include_references: bool, save_to_file: bool, progress=gr.Progress()
):
    """
...
"""
        ...
        if save_to_file:
            filename = (
                f"report_{topic.replace(' ', '_').replace('/', '_')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
            )
        ...
        verified = sum(1 for fc in result["fact_checks"] if fc.get("status") in ["VERIFIED", "SUPPORTED"])
...
**Status:** {"âœ… High Quality Report" if accuracy >= 80 else "âœ… Verified Report" if accuracy >= 60 else "âš "}
        ...
        with gr.TabItem("ðŸ¤– Automated Generation", id=0):
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    topic_description = gr.Markdown(f"*{AVAILABLE_TOPICS['Agentic AI']}*")
                    ...
                    include_refs_auto = gr.Checkbox(label="ðŸ“š Include reference list", value=True)

                    save_file_auto = gr.Checkbox(
                        label="ðŸ’¾ Save to .txt file", value=True, info="Saves with timestamp"
                    )

                    generate_btn = gr.Button(
                        "ðŸš€ Generate Report", variant="primary", size="lg", elem_classes="generate-btn"
                    )
                ...
                with gr.Column(scale=2):
                    ...
                    report_display_auto = gr.Markdown(value="*Your report will appear here after generation...*")

            # Update description when topic changes
            topic_dropdown.change(
                fn=lambda t: f"*{AVAILABLE_TOPICS[t]}*", inputs=[topic_dropdown], outputs=[topic_description]
            )
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    topic_hitl = gr.Dropdown(
                        choices=list(AVAILABLE_TOPICS.keys()), value="Agentic AI", label="ðŸ“š Topic"
                    )

                    temp_hitl = gr.Slider(minimum=0.0, maximum=1.0, value=0.5, step=0.1, label="ðŸŒ¡ï¸ Temperature")

                    draft_btn = gr.Button("ðŸ“ Generate Draft", variant="secondary")
                    ...
                    include_refs_hitl = gr.Checkbox(label="ðŸ“š Include references", value=True)

                    save_hitl = gr.Checkbox(label="ðŸ’¾ Save final version", value=True)

                    finalize_btn = gr.Button("âœ… Finalize Edited Report", variant="primary")
