...
class FraudDataResource(dg.ConfigurableResource):
    ...
    test_size: float = pyd.Field(default=0.2, description="Proportion of data to use for testing")
    random_state: int = pyd.Field(default=42, description="Random state for reproducibility in data splitting")
...
class RandomForestConfig(dg.Config):
    ...
    cv_folds: int = pyd.Field(default=3, description="Number of cross-validation folds for hyperparameter tuning")
    scoring: str = pyd.Field(default="f1", description="Scoring metric for cross-validation")
    random_state: int = pyd.Field(default=42, description="Random state for reproducibility in model training")
