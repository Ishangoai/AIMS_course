...
try:
    ...
    print("Make sure:")
    print("  1. The Dagster pipeline has been run completely")
    print("  2. The model is promoted to Production stage")
    print("  3. MLflow database path is correct")
    raise
...
def predict_fraud(time, amount, v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, 
    ...
    try:
        ...
        if prediction == 1:
            ...
            risk_html = '<div style="background-color: #ff4444; color: white; padding: 20px; border-radius: 10px; text-align: center; font-size: 24px; font-weight: bold;">⚠️ HIGH RISK - FRAUD</div>'
        else:
            result_text = "✅ LEGITIMATE TRANSACTION"
            confidence_text = "This transaction appears to be legitimate."
            risk_html = '<div style="background-color: #44ff44; color: black; padding: 20px; border-radius: 10px; text-align: center; font-size: 24px; font-weight: bold;">✓ LOW RISK - NORMAL</div>'
        ...
        error_html = f'<div style="background-color: #ff8800; color: white; padding: 20px; border-radius: 10px; text-align: center;">⚠️ ERROR: {str(e)}</div>'
...
with gr.Blocks(title="Fraud Detection System", theme=gr.themes.Soft()) as iface:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            risk_output = gr.HTML(
                value='<div style="background-color: #dddddd; color: black; padding: 20px; border-radius: 10px; text-align: center;">Waiting for prediction...</div>',
