...
def get_recommendations_for_user(username=CURRENT_USER, top_n=18):
    ...
    for item_idx in range(num_items):
        ...
        score = (
            ...
            + 0.05 * item_biases[item_idx]  # type: ignore
        )
...
def create_movie_card_html(movie, page_type="popular", show_user_rating=False):
    ...
    if page_type in ["popular", "recommendations"]:
        rating_display = f'<div style="color: #999; font-size: 12px; margin-top: 5px;">⭐ {RATING_COUNTS.get(movie["movieId"], 0)} ratings</div>'  # noqa: E501
...
def create_detail_html(movie_id):
    ...
    if is_recommended:
        ...
        if why_movies:
            ...
            movies_text = ", ".join(movies_list[:-1]) + (" and " if len(movies_list) > 1 else "") + movies_list[-1] if movies_list else ""  # noqa: E501
...
with gr.Blocks(title="🎬 MOVIEFLIX", theme=gr.themes.Soft(), css=custom_css) as app_scale:  # type: ignore
    ...
    def show_rated_movies():
        ...
        for i in range(18):
            if i < len(movies):
                ...
                updates.append(card_html)
                updates.append(gr.update(visible=True))  # pyright: ignore[reportArgumentType]
                updates.append(movie['movieId'])  # Update the state
            else:
                updates.append("")
                updates.append(gr.update(visible=False))  # pyright: ignore[reportArgumentType]
                updates.append(None)  # pyright: ignore[reportArgumentType] # Clear the state
