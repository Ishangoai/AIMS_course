...
def transformed_fraud_data(
    ...
    if (hasattr(scaler, 'mean_') and scaler.mean_ is not None and len(scaler.mean_) > 0 and
        hasattr(scaler, 'scale_') and scaler.scale_ is not None and len(scaler.scale_) > 0):
        amount_mean = float(scaler.mean_[0])
        amount_std = float(scaler.scale_[0])
...
def data_splits(
    ...
    train_fraud_rate = float((y_train.sum() / len(y_train)) * 100 if len(y_train) > 0 else 0.0)
    test_fraud_rate = float((y_test.sum() / len(y_test)) * 100 if len(y_test) > 0 else 0.0)
...
def tune_hyperparameters(
    ...
    for max_depth in model_config.max_depth_options:
        ...
        with mlflow_client.start_run(run_name=run_name, nested=True) as child_run:
...
def evaluate_model(
    ...
    predictions_proba = train_model.predict_proba(X_test)[:, 1]
...
def raw_data_quality_checks(raw_fraud_data: pd.DataFrame) -> Iterable[dg.AssetCheckResult]:
    ...
    is_valid = np.all(np.isin(unique_classes, [0, 1]))
    yield dg.AssetCheckResult(
