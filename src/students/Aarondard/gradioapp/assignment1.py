...
def download_nltk_resources():
    ...
    for resource in resources:
        try:
            nltk.data.find(f'tokenizers/{resource}' if resource in ['punkt', 'punkt_tab'] else
                          f'corpora/{resource}' if resource in ['stopwords'] else
                          f'taggers/{resource}')
        except LookupError:
...
def get_sentences(text):
    try:
        ...
        print(f"unable to fetch sentenses {str(e)}")
...
def average_sentence_length(text):
    ...
    if not sentences:
        return 0
    total_words = sum(len([word for word in get_word_tokens(sent) if word.isalpha()]) for sent in sentences)
...
def flesch_reading_ease(text):
    ...
    total_words = sum(len([word for word in get_word_tokens(sent) if word.isalpha()]) for sent in sentences)
    total_syllables = (sum(count_syllables(word) for sent in sentences
    for word in get_word_tokens(sent) if word.isalpha()))

    if total_words == 0:
        return 0

    return round(206.835 - 1.015 * (total_words / total_sentences) - 84.6 * (total_syllables / total_words), 1)
...
def filter_function_words(words):
...
def create_wordcloud_figure(text):
    ...
    if not filtered_tokens:
        fig, ax = plt.subplots(figsize=(10, 6))
        ax.text(0.5, 0.5, 'No meaningful words to display\n(after filtering common words)',
...
def generate_comprehensive_table(text):
    ...
    return pd.DataFrame(data, columns=["Category", "Statistic", "Value"])


def create_statistics_visualizations(text):
    if not text or text.isspace():
        empty_plot = go.Figure()
        empty_plot.add_annotation(text="No data to display", x=0.5, y=0.5, showarrow=False)
    ...
    stats_fig = go.Figure(data=[
        go.Bar(name='Count', x=['Words', 'Characters', 'Paragraphs', 'Sentences'],
               y=[wc, cc, pc, sc], marker_color=['#FF6B6B', '#4ECDC4', '#45B7D1', '#FFA07A'])
    ])
...
def create_advanced_visualizations(text):
    if not text or text.isspace():
        empty_plot = go.Figure()
        empty_plot.add_annotation(text="No data to display", x=0.5, y=0.5, showarrow=False)
        empty_plot.update_layout(title="No data available")
    ...
    if not filtered_tokens:
        empty_plot = go.Figure()
        empty_plot.add_annotation(text="No meaningful words after filtering", x=0.5, y=0.5, showarrow=False)
    ...
    advanced_fig = go.Figure(data=[
        go.Bar(x=words, y=counts, marker_color='#FFA07A', name='Word Frequency')
    ])
...
def text_actions(text):
    ...
    return processed_text, stats_table, stats_fig, gauge_fig, advanced_fig, wordcloud_fig
...
def clear_all():
    ...
    empty_wordcloud = create_wordcloud_figure("")
    empty_table = generate_comprehensive_table("")
    return "", empty_table, empty_plot, empty_plot, empty_plot, empty_wordcloud, None, False, False
...
""") as dard:
    ...
    send_btn.click(
        fn=text_actions,
        inputs=user_input,
        outputs=[output, stats_display, stats_plot, gauge_plot, advanced_plot, wordcloud_display]
    )
    ...
    clear_all_btn.click(
        ...
        outputs=[user_input, stats_display, stats_plot, gauge_plot, advanced_plot, wordcloud_display,
                case_choice, reverse_word_checkbox, reverse_char_checkbox]
    )
