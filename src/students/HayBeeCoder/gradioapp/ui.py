...
def create_interface():
    ...
    with gr.Blocks(
        ...
        with gr.Accordion("Basic Features", open=False):
            ...
            with gr.Row():
                rotation = gr.Slider(
                    -180, 180, value=0, step=1, label="Rotation (degrees)", info="Rotate the image by degrees."
                )
        ...
        with gr.Accordion("Crazy Features", open=False):
            ...
            with gr.Row():
                apply_kaleidoscope = gr.Checkbox(
                    label="Kaleidoscope", value=False, info="Apply a symmetrical kaleidoscope effect."
                )
                segments = gr.Slider(
                    2, 12, value=4, step=2, label="Kaleidoscope Segments", info="Number of mirrored segments."
                )
            ...
            with gr.Row():
                apply_mosaic = gr.Checkbox(
                    label="Mosaic Effect", value=False, info="Shuffle image tiles for a mosaic effect."
                )
        ...
        with gr.Accordion("Add and Edit Text", open=False):
            ...
            with gr.Row():
                ...
                make_gif = gr.Checkbox(
                    label="GIF-fy", value=False, info="Turn into animated GIF with rotation.", visible=False
                )
            ...
            with gr.Row():
                ...
                text_color = gr.ColorPicker(
                    label=" Text Color", value="#000000", info="Select the color of the text overlay."
                )
                text_font_size = gr.Slider(
                    10, 200, value=50, step=5, label=" Text Font Size", info="Adjust the font size of the overlay text."
                )
        ...
        ascii_output = gr.Textbox(
            label="ASCII Art", interactive=False, lines=10, placeholder="Generate ASCII art from your edited image..."
        )
        ...
        for comp in params:
            comp.change(update_output, all_inputs, [output_img, cloud_url])

        reset_btn.click(reset, None, params).then(update_output, all_inputs, [output_img, cloud_url])

        randomize_btn.click(randomize, None, params).then(update_output, all_inputs, [output_img, cloud_url])

        presets.change(apply_preset, presets, params).then(update_output, all_inputs, [output_img, cloud_url])

        ascii_btn.click(generate_ascii, all_inputs, ascii_output)
