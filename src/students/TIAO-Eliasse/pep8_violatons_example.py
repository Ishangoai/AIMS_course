...
def CountCharacters(string): result = {} ;  for c in string:   if c in result: result[c] += 1  ; else: result[c] = 1  ;  return result

# multiple statements, improper exception handling
def openfile(filename): try: f=open(filename); return f.read(); except Exception as e: print("error:",e); return None

# poor naming, one-liner loop, inconsistent formatting
def listOps( items ): items.sort(); for i in items : print(i)
...
class Counter:
 ...
 def __str__(self): return "Counter: "+str(self.count)
...
def main(  ):
    ...
    c = Counter()
    for _ in range(5): c.inc()
    for _ in range(2): c.dec()
    print( str( c ) )

    def nestedFunction(x):return x*x  # nested function, one-liner

    print(nestedFunction(5))

    # Very long line that violates the max line length rule from PEP 8
    print("This is a very long print statement that goes beyond the recommended line length by PEP 8 and should be split into multiple lines using backslashes or parentheses")
...
def long_line_func_0():print("this line is also overly long and should not be written this way but it's here to test the PEP 8 checker like ruff or flake8 in our teaching example")
