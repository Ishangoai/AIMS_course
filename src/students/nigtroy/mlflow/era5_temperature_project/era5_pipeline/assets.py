...
def fetch_era5_data(context: dg.AssetExecutionContext) -> str:
    ...
    try:
        ...
        c.retrieve("reanalysis-era5-single-levels", ERA5_REQUEST_PARAMS, OUTPUT_FILENAME)
...
def process_temperature_dataframe(
    context: dg.AssetExecutionContext, raw_era5_temperature_data: str
...
def clean_temperature_data_pandas(
    context: dg.AssetExecutionContext, processed_temperature_dataframe: pd.DataFrame
) -> dg.MaterializeResult:
    mlflow_client = context.resources.mlflow_tracking
    ...
    if not df.empty:
        ...
        try:
            ...
            cleaned_csv_as_input = mlflow_client.data.from_files(
                path=dataset_source_uri, name="cleaned_temperature_sample_csv"
            )
...
def tune_ridge_hyperparameters(
    context: dg.AssetExecutionContext,  # noqa: C901
